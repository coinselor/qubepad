...
export async function fetchAndUpsertPillars() {
	try {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to fetch pillars: ${response.status} ${response.statusText}`
			);
		}
			...
			try {
				...
				if (!publicKeyHex) {
					...
					if (!frontierResponse.ok) {
						console.error(
							`Failed to fetch frontier data for ${pillar.name}: ${frontierResponse.status} ${frontierResponse.statusText}`
						);
						continue;
					}

					const contentType =
						frontierResponse.headers.get("content-type");
					if (
						!contentType ||
						!contentType.includes("application/json")
					) {
						console.error(
							`Expected JSON response for frontier data but received: ${contentType}`
						);
						continue;
					}

					const frontierData: FrontierAccountBlockResponse =
						await frontierResponse.json();
				...
				await db
					...
					.values(
						pillar as Required<
							Pick<
								Pillar,
								| "alphanet_pillar_name"
								| "alphanet_pillar_address"
								| "weight"
								| "alphanet_pillar_public_key"
								| "updated_at"
							>
						>
					)
