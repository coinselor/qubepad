...
export class ZnnAddress {
  ...
  static fromPublicKey(publicKey: Buffer): string {
    ...
    const core = Buffer.concat([Buffer.from([this.USER_BYTE]), Buffer.from(digest)]);
   ...
   */
  private static parseAddress(address: string): { prefix: string; core: Buffer } {
    const { prefix, words } = bech32.decode(address);

    if (prefix !== this.PREFIX) {
      throw new Error(`Invalid prefix ${prefix}; should be ${this.PREFIX}`);
    }

    const core = Buffer.from(bech32.fromWords(words));
    if (core.length !== this.CORE_SIZE) {
      throw new Error(`Invalid length ${core.length}; should be ${this.CORE_SIZE}`);
    }
